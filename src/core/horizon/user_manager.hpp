...
class User {
  ...
  public:
    ...
    // Getters
    u64 GetLastEditTimestamp() const { return last_edit_timestamp; }
  ...
  private:
    ...
    User(const std::string& nickname_) : nickname{nickname_} { NotifyEdit(); }

    // Helpers
    void NotifyEdit() { last_edit_timestamp = get_absolute_time(); }
};
