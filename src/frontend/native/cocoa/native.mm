...
- (void)showTextInputDialog:(NSString*)title
    ...
    // Create text field
    self.textField = [[NSTextField alloc] initWithFrame:NSMakeRect(0, 0, 300, 24)];
...
Native::Native() {
    text_input_delegate = [[TextInputDelegate alloc] init];
}

Native::~Native() {
    [text_input_delegate release];
}

bool Native::ShowInputTextDialog(const std::string& header_text,
                         std::string& out_text) {
    NSString* ns_title = [NSString stringWithUTF8String:header_text.c_str()];
    NSString* ns_placeholder = @"";

    Result result;
    auto result_ptr = &result; // Take a reference to result to not confuse the compiler
