...
class IostreamStream : public IStream {
  public:
    ...
    u64 GetSeek() const override { return stream.tellg(); }
    void SeekTo(u64 seek) override { stream.seekg(seek, std::ios::beg); }
    void SeekBy(u64 offset) override { stream.seekg(offset, std::ios::cur); }
